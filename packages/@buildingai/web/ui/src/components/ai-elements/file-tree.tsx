...
import { Collapsible, CollapsibleContent } from "@buildingai/ui/components/ui/collapsible";
      ...
      <Collapsible open={isExpanded}>
        <div className={cn("", className)} role="treeitem" tabIndex={0} {...props}>
          ...
          >
            ...
            <FileTreeIcon>
              {isExpanded
                ? (expandedIcon ?? <FolderOpenIcon className="size-4 text-blue-500" />)
                : (icon ?? <FolderIcon className="size-4 text-blue-500" />)}
            </FileTreeIcon>
            <FileTreeName className={cn("flex-1", nameClassName)} title={nameTitle ?? name}>
              {name}
            </FileTreeName>
            {trailing}
            {actions}
